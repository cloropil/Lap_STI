...
<div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
    ...
    <div class="px-6 py-6">
        ...
        <form action="{{ route('lokasi.update', $lokasi->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="no" class="block text-base font-medium text-gray-600 mb-1">No Urut</label>
                <input type="number" name="no" id="no" value="{{ $lokasi->no }}"
            ...
            <div>
                ...
                <input type="text" name="lokasi" id="lokasi" value="{{ old('lokasi', $lokasi->lokasi) }}"
            ...
            <div>
                ...
                <input type="text" name="sid" id="sid" value="{{ old('sid', $lokasi->sid) }}"
                    class="w-full border border-gray-300 rounded-md text-base px-2 py-1.5" required>
            </div>
            ...
            <div>
                ...
                <select name="product" id="product"
                    ...
                    <option value="SCADA" {{ old('product', $lokasi->product) == 'SCADA' ? 'selected' : '' }}>SCADA</option>
                    <option value="WAN Office" {{ old('product', $lokasi->product) == 'WAN Office' ? 'selected' : '' }}>WAN Office</option>
                </select>
            </div>

            <div>
                <label for="bandwith" class="block text-base font-medium text-gray-600 mb-1">Bandwidth</label>
                <input type="text" name="bandwith" id="bandwith" value="{{ old('bandwith', $lokasi->bandwith) }}"
            ...
            <div>
                ...
                <select name="kategori_layanan" id="kategori_layanan"
                    class="w-full border border-gray-300 rounded-md text-base px-2 py-1.5">
                    <option value="">-- Pilih --</option>
                    <option value="INTERNET" {{ old('kategori_layanan', $lokasi->kategori_layanan) == 'INTERNET' ? 'selected' : '' }}>INTERNET</option>
                    <option value="IPVPN" {{ old('kategori_layanan', $lokasi->kategori_layanan) == 'IPVPN' ? 'selected' : '' }}>IPVPN</option>
                    <option value="IPVSAT" {{ old('kategori_layanan', $lokasi->kategori_layanan) == 'IPVSAT' ? 'selected' : '' }}>IPVSAT</option>
                    <option value="CLEARCHANNEL" {{ old('kategori_layanan', $lokasi->kategori_layanan) == 'CLEARCHANNEL' ? 'selected' : '' }}>CLEAR CHANNEL</option>
                    <option value="METRONET" {{ old('kategori_layanan', $lokasi->kategori_layanan) == 'METRONET' ? 'selected' : '' }}>METRONET</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="standard_availability" id="standard_availability"
                    value="{{ old('standard_availability', $lokasi->standard_availability) }}"
                    class="w-full border border-gray-300 rounded-md text-base px-2 py-1.5">
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="realisasi_availability" id="realisasi_availability"
                    value="{{ old('realisasi_availability', $lokasi->realisasi_availability) }}"
                    class="w-full border border-gray-300 rounded-md text-base px-2 py-1.5">
            </div>
            ...
            <div class="pt-4 flex justify-between">
                ...
                <button type="submit"
                    class="px-5 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-base">Perbarui</button>
            </div>
        </form>
    </div>
</div>
